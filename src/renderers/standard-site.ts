 ...
 */
export const standardSiteDocumentRenderer: RendererFunction<site.standard.document.Main> = async (
  entry,
) => {
  const { content } = entry
  ...
  if (content.$type === "app.offprint.content")
    return import("./offprint").then((m) =>
      m.contentRenderer(content as unknown as app.offprint.content.Main),
    )
